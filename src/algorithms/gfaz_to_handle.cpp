...
static void add_edge_elements(const GfaGraph &gfaz_graph,
  ...
  for (size_t i = 0; i < edge_count; ++i) {
    ...
    if (!valid_segment_id(gfaz_graph, from) ||
        !valid_segment_id(gfaz_graph, to)) {
...
template<class T>
static inline void release_vector(vector<T> &value) {
  vector<T>().swap(value);
}

static inline void release_node_name_map(unordered_map<string, uint32_t> &value) {
...
parse_reference_samples(const GfaGraph &gfaz_graph) {
...
static void add_w_line_paths(const GfaGraph &gfaz_graph,
  ...
  for (size_t i = 0; i < walks.walks.size(); ++i) {
    const string sample_name =
        (i < walks.sample_ids.size()) ? walks.sample_ids[i] : "*";
    const size_t haplotype =
        (i < walks.hap_indices.size()) ? walks.hap_indices[i] : 0;
    const string contig_name = (i < walks.seq_ids.size())
                                   ? walks.seq_ids[i]
                                   : PathMetadata::NO_LOCUS_NAME;
    const int64_t start = (i < walks.seq_starts.size())
                              ? walks.seq_starts[i]
                              : PathMetadata::NO_END_POSITION;
    const int64_t end = (i < walks.seq_ends.size())
                            ? walks.seq_ends[i]
                            : PathMetadata::NO_END_POSITION;
...
static void add_rgfa_paths(const GfaGraph &gfaz_graph,
  ...
  for (nid_t node_id = 1; static_cast<size_t>(node_id) <= segment_count(gfaz_graph);
       ++node_id) {
    ...
    size_t length =
        (static_cast<size_t>(node_id) < node_lengths.size()) ? node_lengths[node_id] : 0;
...
void gfaz_to_path_handle_graph(const string &filename,
  ...
  GfaGraph gfaz_graph = load_gfaz_graph(filename, num_threads);
  set_numeric_translation(gfaz_graph, translation);
  vector<size_t> node_lengths(gfaz_graph.node_sequences.size(), 0);
  for (size_t i = 1; i < gfaz_graph.node_sequences.size(); ++i) {
    node_lengths[i] = gfaz_graph.node_sequences[i].size();
  }
  add_node_elements(gfaz_graph, graph);
  release_segment_sequences(gfaz_graph);
  add_edge_elements(gfaz_graph, graph);
  ...
  release_paths(gfaz_graph);
  add_w_line_paths(gfaz_graph, graph, reference_samples);
  release_walks(gfaz_graph);
  add_rgfa_paths(gfaz_graph, graph, node_lengths, max_rgfa_rank);
  release_vector(node_lengths);
  release_segment_metadata(gfaz_graph);
}
